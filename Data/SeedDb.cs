...
namespace CandyShop.Data
{
    public class SeedDb
    {
        ...
        public static void InitializeDbIfEmpty(IServiceProvider services)
        {
            var dbContext = services.GetService<CandyShopDbContext>();

            //  dbContext.Database.EnsureCreated() --> Checks if database exists. If not, it creates the database.
            // dbContext.Database.EnsureCreated();

            // dbContext.Database.Migrate(); --> Checks if database exists. If not, it creates the database AND Migrate 
            // everything necessary in order to build the Model from CandyShopDb database.
            dbContext.Database.Migrate();


            //1) Initialize Categories Entities
            ...
            //2) Initialize Candies Entities
            if (!dbContext.Candies.Any())
            {
